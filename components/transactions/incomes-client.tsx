...
export function IncomesClient() {
  ...
  const handleSave = async (formData: any) => {
    if (editItem) {
      const res = await fetch(`/api/incomes/${editItem.id}`, {
        method: 'PUT',
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await fetch(`/api/incomes/${deleteId}`, { method: 'DELETE' });
    ...
    <div className="space-y-6">
      <FadeIn>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="font-display text-2xl font-bold tracking-tight">Rendas</h1>
            <p className="text-muted-foreground text-sm mt-1">Gerencie todas as suas fontes de renda.</p>
          </div>
          <Button onClick={() => { setEditItem(null); setModalOpen(true); }}>
            <Plus className="w-4 h-4 mr-2" /> Adicionar Renda
          </Button>
        </div>
      </FadeIn>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Select value={categoryFilter} onValueChange={(v: string) => setCategoryFilter(v)}>
            ...
            <SelectContent>
              ...
              {INCOME_CATEGORIES.map((c: string) => (
          ...
          <Card className="ml-auto hidden sm:flex items-center px-4 py-2">
            ...
            <span className="ml-1 font-mono font-bold text-emerald-500">{formatCurrency(total)}</span>
          </Card>
        </div>
      </FadeIn>
            ...
            {loading ? (
              ...
              <div className="p-12 text-center text-muted-foreground">
                ...
                <Button variant="outline" className="mt-4" onClick={() => { setEditItem(null); setModalOpen(true); }}>
                  <Plus className="w-4 h-4 mr-2" /> Adicionar primeira renda
                </Button>
              </div>
            ) : (
                    ...
                    {(incomes ?? []).map((item: Income) => (
                      <tr key={item?.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-sm px-2 py-1 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-medium">{item?.category}</span>
                        </td>
                        <td className="px-4 py-3 text-sm text-muted-foreground hidden sm:table-cell">{item?.description || '—'}</td>
                        <td className="px-4 py-3 text-right font-mono font-semibold text-sm text-emerald-500">{formatCurrency(item?.amount)}</td>
      ...
      <div className="sm:hidden">
        <Card>
          <CardContent className="p-4 flex items-center justify-between">
            <span className="text-sm font-medium">Total de Rendas</span>
            <span className="font-mono font-bold text-emerald-500">{formatCurrency(total)}</span>
          </CardContent>
        </Card>
      </div>
      ...
      <TransactionModal
        ...
        onSave={handleSave}
        title={editItem ? 'Editar Renda' : 'Adicionar Renda'}
